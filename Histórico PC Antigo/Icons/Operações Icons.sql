SELECT A.emps + A.notas AS 'CHAVE', A.EMPDOPNUMS AS 'CHAVE_MAE', A.emps AS 'EMPRESA', DATEFROMPARTS(YEAR(A.datas),MONTH(A.datas), 1) AS 'DATA_REF',
	...
	CAST(A.obses AS varchar(max)) AS 'OBSERVÇÃO', N.empdncrds AS 'FINALIZACAO', P.descs AS 'LINHA'
...
LEFT JOIN SIGCDFIP P (NOLOCK) ON P.cods = D.codfinp 
WHERE A.DOPES IN ( 'NF OUTRAS SAIDAS DI.', 'NF REMESSA CONSIGNAD','NF OUTRAS SAIDAS DIV','NF ENT DEMONSTRAÇÃO ','NF SAIDA DEMONSTRA  ','NF DEV CONSIGNADO   ', 'NF VENDA')
	AND C.citem2 = 0
...
GROUP BY A.cidchaves, A.notas, A.dopes, A.numes, F.rclis, A.nops, A.datas, A.emps, A.empdopnums, CAST(A.obses AS varchar(max)), J.rclis,
				...
				C.qtbaixas, C.units, C.valipis, C.valdescs , N.datas , O.compet , O.datas , O.prazoents, P.descs

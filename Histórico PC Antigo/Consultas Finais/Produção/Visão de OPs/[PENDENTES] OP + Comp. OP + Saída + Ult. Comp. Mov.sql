SELECT DISTINCT A.dopes as 'TIPO DE PEDIDO', B.rclis as 'CLIENTE', A.mascnum as 'PEDIDO', A.nops AS 'OP_PEDIDO', D.NOPS AS 'OP_ITEM', D.nopmaes AS 'OP_MAE',
				...
				M.datas AS 'ULT_MOV_CONC', M.tpops AS 'ULT_ATIVI_CONC', R.iclis AS 'CÓD_CONTA_APURACAO', R.rclis AS 'CONTA_APURACAO',
	...
	INNER JOIN (select a.grupods, a.contads, a.grupoos, a.contaos, a.datas, a.numps, a.dopps, a.emps, a.nops, a.QTDS, a.empdnps, a.cidchaves, C.iclis, C.rclis
					from sigpdmvf a (NOLOCK)
						join (select aa.nops, MAX(aa.cidchaves) as cidchaves
									from SigPdMvf aa (NOLOCK)
										left join SigCdNei bb (NOLOCK) ON aa.empdnps = bb.empdnps
										WHERE aa.nops NOT IN(SELECT DISTINCT aa.NOPS FROM SIGPDMVF where aa.dopps in
												('FINALIZAÇÃO','FINALIZA S INDUSTRIA','FINALIZA OP S/BARRA '))
												AND bb.tpops IN ('OURIVESARIA.TRA')
										group by aa.nops 
								) B ON A.nops = B.nops AND A.cidchaves = B.cidchaves
						inner join SIGCDCLI C ON A.contaos = C.iclis
	 					where dopps <> SPACE(20)
				) R on d.nops = R.nops
	...
	GROUP BY A.dopes, B.rclis, A.mascnum, A.nops, D.NOPS, C.CPROS, D.dpros, G.codcors, A.datas, A.PRAZOENTS, C.qtds,H.cidchaves,D.qtds, E.grupoos,
	 			...
	 			Q.grupos, Q.minutos, Convert(varchar(max), Q.obs), R.iclis, R.rclis

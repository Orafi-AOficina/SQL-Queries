SELECT C.codbarras , A.cidchaves AS 'UKEY_NF', A.emps + A.notas AS 'CHAVE', A.EMPDOPNUMS AS 'CHAVE_MAE', A.datas AS 'DATA',
...
WHERE A.DOPES IN ('NF RET INDUSTRIALIZA', 'NF VENDA', 'NF VENDA PILOTO', 'DV ASS. TEC. C.CUSTO',
			...
			'NF RET PURIFICA플O', 'NF ENT INDUSTRIA')cdcli
	AND C.citem2 = 0
	AND A.datas >='2018-07-01'
	AND (G.stats NOT LIKE '' OR G.stats IS NULL)
GROUP BY A.cidchaves, A.notas, A.dopes, A.numes, F.rclis, A.nops, A.datas, A.emps, A.empdopnums, CAST(A.obses AS varchar(max)), J.rclis, C.codbarras

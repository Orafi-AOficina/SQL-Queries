WITH ops_pendentes AS (
    SELECT nops
    FROM (
        SELECT nops, dopps,
               ROW_NUMBER() OVER (PARTITION BY nops ORDER BY cidchaves DESC) AS rn,
               MAX(CASE WHEN dopps IN ('FINALIZAÇÃO','FINALIZA S INDUSTRIA',
                                        'FINALIZA OP S/BARRA ','FINALIZAÇÃO OS')
                        THEN 1 ELSE 0 END) OVER (PARTITION BY nops) AS tem_finalizacao
        FROM SIGPDMVF (NOLOCK)
    ) x
    WHERE rn = 1 AND tem_finalizacao = 0
),

hierarquia AS (
    SELECT nops, nopmaes, SUM(qtds) AS qtds_total, dopes
    FROM SIGOPPIC (NOLOCK)
    GROUP BY nops, nopmaes, dopes
),
...
resumo_op AS (
    SELECT 
        CASE
            ...
            WHEN COALESCE(P2.nopmaes, 0) = 0           THEN P2.nops

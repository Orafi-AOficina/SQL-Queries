SELECT DISTINCT A.dopes as 'TIPO DE PEDIDO', B.rclis as 'CLIENTE', A.mascnum as 'PEDIDO', A.nops AS 'OP_PEDIDO', D.NOPS AS 'OP_ITEM', D.nopmaes AS 'OP_MAE',
				...
				END AS 'QTD_COMP',
				H.pesos AS 'PESOS_COMP', H.qtds AS 'QTDS_COMP', H.univals AS 'PREÇO',
				CASE WHEN LEFT(D.dpros,4) IN ('TARR', 'MOSQ') THEN 'VERDADEIRO' ELSE 'FALSO' END AS 'TARRAXA/MOSQUETAO', Convert(varchar(max),a.obses) as 'OBSERVAÇÃO'
	...
	FULL JOIN SigCdNei J (NOLOCK) ON E.empdnps = J.empdnps AND J.nops = D.nops AND (J.cmats = I.cpros OR (J.cmats = 'AU750' AND I.cpros LIKE 'RODIO%') OR (J.cmats = 'AG925' AND I.cpros LIKE 'RODIO%'))
	WHERE A.dopes IN ('pedido FABRICA','PEDIDO ENCOMENDA','PEDIDO PILOTO','PED ACRESC PRODUCAO ','ENTRADA CONSERTO','ENTRADA CONSERTO    ',
	 			...
	 			--AND A.nops = 7309
	 			--AND D.nops IN ('69770004')
	GROUP BY A.dopes, B.rclis, A.mascnum, A.nops, D.NOPS, C.CPROS, D.dpros, G.codcors, A.datas, A.PRAZOENTS, C.qtds,H.cidchaves,D.qtds, E.grupods,
	 			E.contads, E.DATAS, Convert(varchar(max),a.obses), G.reffs, H.cpros, H.univals, I.cgrus,  H.qtds, C.empdopnums, C.citem2, C.citens,
	 			D.nopmaes, H.pesos, I.cpros, I.dpros
